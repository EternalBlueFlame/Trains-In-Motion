...
public class ModelRendererTurbo {
    ...
    private List<TexturedPolygon> faces;
    ...
    public void addShape3D(float x, float y, float z, Shape2D shape, float depth, int shapeTextureWidth, int shapeTextureHeight, int sideTextureWidth, int sideTextureHeight, float rotX, float rotY, float rotZ, float[] faceLengths){
        copyTo(shape.extrude(-x, y, -z, rotX, rotY, rotZ, depth, textureOffsetX, textureOffsetY, textureWidth, textureHeight, shapeTextureWidth, shapeTextureHeight, sideTextureWidth, sideTextureHeight, faceLengths));
    }
    ...
    public void addCone(float x, float y, float z, float radius, float length, int segments, float baseScale, int baseDirection, int textureCircleDiameterW, int textureCircleDiameterH){
        addCylinder(x, y, z, radius, length, segments, baseScale, 0.0F, baseDirection, textureCircleDiameterW, textureCircleDiameterH, 1,null);
    }
    ...
    public ModelRendererTurbo addCylinder(float x, float y, float z, float radius, float length, int segments, float baseScale, float topScale, int baseDirection, int textureCircleDiameterW, int textureCircleDiameterH, int textureH, Vec3f topoff){
        ...
        List<TexturedVertex> tempVerts = new ArrayList<>();
        ...
        tempVerts.add(new TexturedVertex(xStart, yStart, zStart, 0, 0));
        for(int repeat = 0; repeat < (coneBase || coneTop ? 1 : 2); repeat++){
            for(int index = 0; index < segments; index++){
                ...
                tempVerts.add(new TexturedVertex(xPlace, yPlace, zPlace, 0, 0 ));
            }
            xCur = xEnd; yCur = yEnd; zCur = zEnd; sCur = topScale;
        }
        //tempVerts.add(new TexturedVertex(xEnd, yEnd, zEnd, 0, 0));
        ...
        for(int index = 0; index < segments; index++){
            ...
            vert[0] = tempVerts.get(0).setTexturePosition(uStart + 0.5F * uCircle, vStart + 0.5F * vCircle);
            vert[1] = tempVerts.get(1 + index2).setTexturePosition(uStart + 0.5F * uCircle + uSize1, vStart + 0.5F * vCircle + vSize1);
            vert[2] = tempVerts.get(1 + index).setTexturePosition(uStart + 0.5F * uCircle + uSize, vStart + 0.5F * vCircle + vSize);
            ...
            if(!coneBase && !coneTop){
                ...
                vert[0] = tempVerts.get(1 + index).setTexturePosition(uStart + uOffset + uWidth * index, vStart + vOffset + vCircle);
                vert[1] = tempVerts.get(1 + index2).setTexturePosition(uStart + uOffset + uWidth * (index + 1), vStart + vOffset + vCircle);
                vert[2] = tempVerts.get(1 + segments + index2).setTexturePosition(uStart + uOffset + uWidth * (index + 1), vStart + vOffset + vCircle + vHeight);
                vert[3] = tempVerts.get(1 + segments + index).setTexturePosition(uStart + uOffset + uWidth * index, vStart + vOffset + vCircle + vHeight);
                poly[index + segments] = new TexturedPolygon(Arrays.asList(vert));
                if(!dirFront || mirror){
                    poly[index + segments].flipFace();
                }
            }
            vert = new TexturedVertex[3];
            vert[0] = tempVerts.get(tempVerts.size() - 1).setTexturePosition(uStart + 1.5F * uCircle, vStart + 0.5F * vCircle);
            vert[1] = tempVerts.get(tempVerts.size() - 2 - index).setTexturePosition(uStart + 1.5F * uCircle + uSize1, vStart + 0.5F * vCircle + vSize1);
            vert[2] = tempVerts.get(tempVerts.size() - (1 + segments) + ((segments - index) % segments)).setTexturePosition(uStart + 1.5F * uCircle + uSize, vStart + 0.5F * vCircle + vSize);
    ...
    public ModelRendererTurbo addHollowCylinder(float x, float y, float z, float radius, float radius2, float length, int segments, int seglimit, float baseScale, float topScale, int baseDirection, int textureCircleDiameterW, int textureCircleDiameterH, int textureH, Vec3f topoff, float topangle, boolean[] bools){
        ...
        List<TexturedVertex> arr = new ArrayList<>();
        for(int repeat = 0; repeat < 2; repeat++){//top/base faces
            ...
            if((repeat == 0 && !bools[0]) || (repeat == 1 && !bools[1])){
                for(int i = 0; i < verts0.size(); i++){
                    ...
                    arr.add(verts0.get(i).setTexturePosition(uStart + mul * uCircle + xSize, vStart + 0.5F * vCircle + ySize));
                    //
                    ...
                    arr.add(verts1.get(i).setTexturePosition(uStart + mul * uCircle + xSize, vStart + 0.5F * vCircle + ySize));
                    //
                    ...
                    arr.add(verts1.get(i + 1).setTexturePosition(uStart + mul * uCircle + xSize, vStart + 0.5F * vCircle + ySize));
                    //
        ...
        for(int i = 0; i < halfv2; i++){
            if(i >= seglimit && segl){
                arr.clear();
                float xpos = uStart + uOffset + (uCircle * 2f);
                arr.add(verts2.get(0).setTexturePosition(xpos, vStart + vOffset + vCircle));
                arr.add(verts3.get(0).setTexturePosition(xpos, vStart + vOffset + vCircle + vHeight));
            ...
            arr.clear();
            if(!bools[2]){
                arr.add(verts2.get(i).setTexturePosition(uStart + uOffset + uWidth * i, vStart + vOffset + vCircle));
                arr.add(verts3.get(i).setTexturePosition(uStart + uOffset + uWidth * i, vStart + vOffset + vCircle + vHeight));
            ...
            if(!bools[3]){
                arr.clear();
                arr.add(verts2.get(i + halfv2).setTexturePosition(uStart + uOffset + uWidth * i, vStart + vOffset + vCircle + vHeight));
                arr.add(verts3.get(i + halfv2).setTexturePosition(uStart + uOffset + uWidth * i, vStart + vOffset + vCircle + vHeight + vHeight));
    ...
    public void addModel(String file, Class<?> modelFormat){
        ...
        copyTo(Arrays.copyOf(entry.faces, entry.faces.length));
    }
    ...
    public ModelRendererTurbo copyTo(List<TexturedPolygon> poly){
        faces.addAll(poly);
        return this;
    }
